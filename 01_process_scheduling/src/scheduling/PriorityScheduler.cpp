...
SimulationResult simulatePriority(const std::vector<ProcessInput>& input) {
    ...
    while (completed < total) {
        ...
        for (int i = 0; i < total; ++i) {
            ...
            if (selected == -1 || process.priority < result.processes[selected].priority ||
                (process.priority == result.processes[selected].priority && process.arrival < result.processes[selected].arrival) ||
                (process.priority == result.processes[selected].priority && process.arrival == result.processes[selected].arrival && process.name < result.processes[selected].name)) {
                selected = i;
            }
        }
